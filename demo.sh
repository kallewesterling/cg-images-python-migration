...
pe "docker rm -f pymigrate >/dev/null 2>&1; docker build -f docker/Dockerfile.v0 -t pymigrate:v0 app"
...
pe "docker rm -f pymigrate; docker build -f docker/Dockerfile.containers -t pymigrate:containers app"
...
pe "docker images pymigrate --format 'table {{.Tag}}\t{{.Size}}'"
...
pe "docker rm -f pymigrate; docker build -f docker/Dockerfile.libraries -t pymigrate:libraries app"
