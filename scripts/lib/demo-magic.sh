...
##
function p() {
  if [[ ${1:0:1} == "#" && "${SKIP_COMMENTS:-false}" == true ]]; then
    return
  fi
  ...
  # indent any continuation lines (embedded newlines) to line up under where
  # the prompt ends, rather than starting flush at column 0.
  plain_prompt=$(printf '%s' "$x" | sed -E 's/\x1b\[[0-9;]*m//g')
  indent=$(printf '%*s' "${#plain_prompt}" '')
  text="${1//$'\n'/$'\n'$indent}"

  if [[ ${text:0:1} == "#" ]]; then
    cmd=$DEMO_COMMENT_COLOR$text$COLOR_RESET
  else
    cmd=$DEMO_CMD_COLOR$text$COLOR_RESET
  fi
